...
class Couple(Base):
    ...
    # Styling Fields
    primary_color = Column(String, default="#4a4e3a")
    secondary_color = Column(String, default="#f9f8f6")
    font_family_serif = Column(String, default="Cormorant Garamond")
    font_family_sans = Column(String, default="Montserrat")
    # Yeni Hibrit Alanlar
...
def create_couple(
    ...
    story_image_url: str = Form(None),
    thankyou_image_url: str = Form(None),
    envelope_text: str = Form(None),
    program_json: str = Form(None),
    primary_color: str = Form("#4a4e3a"),
    secondary_color: str = Form("#f9f8f6"),
    font_family_serif: str = Form("Cormorant Garamond"),
    font_family_sans: str = Form("Montserrat"),
    ...
    # Check if exists (for update logic simplicity)
    existing = db.query(Couple).filter(Couple.slug == slug).first()
    if existing:
        existing.bride_name = bride_name
        existing.groom_name = groom_name
        ...
        existing.story_image_url = story_image_url
        existing.thankyou_image_url = thankyou_image_url
        existing.envelope_text = envelope_text
        existing.program_json = program_json
        existing.primary_color = primary_color
        existing.secondary_color = secondary_color
        existing.font_family_serif = font_family_serif
        existing.font_family_sans = font_family_sans
        existing.is_image_based = is_image_based
        existing.canva_image_url = canva_image_url
        existing.active_sections = active_sections
    else:
        new_couple = Couple(
            ...
            thankyou_image_url=thankyou_image_url,
...
async def preview_invitation(request: Request):
    ...
    from datetime import datetime
    try:
        parsed_date = datetime.strptime(form_data.get("event_date", ""), "%Y-%m-%d").date()
    except:
        parsed_date = datetime.now().date()

    # Get active sections from form
    raw_sections = form_data.get("active_sections", '[]')
    try:
        active_sections_list = json.loads(raw_sections)
    except:
        active_sections_list = ["story", "details", "timeline", "map", "rsvp"]

    class DummyCouple:
        def __init__(self, data):
            self.bride_name = data.get("bride_name", "Gelin")
            self.groom_name = data.get("groom_name", "Damat")
            self.event_date = parsed_date
            self.location_name = data.get("location_name", "")
            self.maps_link = data.get("maps_link", "")
            self.story_text = data.get("story_text", "")
            self.cover_image_url = data.get("cover_image_url", "")
            self.story_image_url = data.get("story_image_url", "")
            self.thankyou_image_url = data.get("thankyou_image_url", "")
            self.envelope_text = data.get("envelope_text", "")
            self.program_json = data.get("program_json", "[]")
            self.primary_color = data.get("primary_color", "#4a4e3a")
            self.secondary_color = data.get("secondary_color", "#f9f8f6")
            self.font_family_serif = data.get("font_family_serif", "Cormorant Garamond")
            self.font_family_sans = data.get("font_family_sans", "Montserrat")
            self.is_image_based = data.get("is_image_based") == "true"
            self.canva_image_url = data.get("canva_image_url", "")
            self.active_sections = raw_sections
            self.slug = data.get("slug", "preview")

    dummy_couple = DummyCouple(form_data)
    
    return templates.TemplateResponse(request=request, name="invitation.html", context={
        "couple": dummy_couple,
        "active_sections": active_sections_list,
        "program": [] # Preview doesn't need full timeline logic
    })

@app.get("/admin/export/{couple_slug}")
def export_rsvp(couple_slug: str, username: str = Depends(get_current_username), db: Session = Depends(get_db)):
    import csv
    import io
    ...
    writer.writerow(["Ad Soyad", "Katılım Durumu", "Kişi Sayısı", "Mesaj"])
    for r in rsvps:
        writer.writerow([r.guest_name, r.attendance_status, r.guest_count, r.message])
    
    output.seek(0)
    
    filename = f"lcv_listesi_{couple_slug}.csv"
